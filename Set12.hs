...
fmap2 h x = fmap (fmap h) x 

fmap3 :: (Functor f, Functor g, Functor h) => (a -> b) -> f (g (h a)) -> f (g (h b))
fmap3 h x = fmap (fmap (fmap h)) x
...
instance Functor List where
  fmap f Empty = Empty 
  fmap f (LNode x ys) = LNode (f x) (fmap f ys)
...
count x ys = foldr (\y n -> if y == x then n+1 else n) 0 ys
...
inBoth xs ys = intersect (folded xs) (folded ys)
  where 
    folded zs = foldr (\z ls -> [z]++ls) [] zs  
...
instance Foldable TwoList where
  ...
  foldr f z (TwoNode x y ts) =  f x (f y (foldr f z ts)) 
