...
blendColor c1 c2 = Color (ave r1 r2) (ave g1 g2) (ave b1 b2)
  where 
    ave p q = (p+q) `div` 2 
    r1 = getRed c1 
    r2 = getRed c2 
...
paintSolid color (Shape s) (Picture p) = Picture (\coord -> if s coord then color else p coord)
------------------------------------------------------------------------------
...
paint (Picture pat) (Shape s) (Picture base) = Picture (\coord -> if s coord then pat coord else base coord)
------------------------------------------------------------------------------
...
instance Transform Fill where
  apply (Fill col) (Picture p) = Picture (\coord -> col)
...
instance Transform Flip where
  ...
  apply (FlipXY) (Picture p) = Picture (\(Coord x y) -> p (Coord y x))
...
data Translate = Translate Int Int
  deriving Show 

instance Transform Translate where 
  apply (Translate n m) (Picture p) = Picture (\(Coord x y) -> p (Coord (x+n) (y+m)))

-- averagePictures :: [Picture] -> Picture 
-- averagePictures [] = solid black 
-- averagePictures xs = Picture (\coord -> ((sumPictures xs) coord) / (fromIntegral (length xs)))

-- sumPictures :: [Picture] -> (Coord -> Color)
-- sumPictures [] = (\coord -> black)
-- sumPictures ((Picture p):xs) = (\coord -> (p coord) + (sumPictures xs) coord)

instance Transform Blur where
  apply Blur (Picture p) = Picture (\coord -> Color (aveRedNeighbors coord) (aveGreenNeighbors coord) (aveBlueNeighbors coord))
    where 
      getRedP coord = getRed (p coord)
      getRedPxp1 (Coord x y) = getRed (p (Coord (x+1) y))
      getRedPxm1 (Coord x y) = getRed (p (Coord (x-1) y))
      getRedPyp1 (Coord x y) = getRed (p (Coord x (y+1)))
      getRedPym1 (Coord x y) = getRed (p (Coord x (y-1)))
      sumRedNeighbors coord = getRedP coord + getRedPxp1 coord + getRedPxm1 coord + getRedPyp1 coord + getRedPym1 coord
      aveRedNeighbors coord = div (sumRedNeighbors coord) 5
      getBlueP coord = getBlue (p coord)
      getBluePxp1 (Coord x y) = getBlue (p (Coord (x+1) y))
      getBluePxm1 (Coord x y) = getBlue (p (Coord (x-1) y))
      getBluePyp1 (Coord x y) = getBlue (p (Coord x (y+1)))
      getBluePym1 (Coord x y) = getBlue (p (Coord x (y-1)))
      sumBlueNeighbors coord = getBlueP coord + getBluePxp1 coord + getBluePxm1 coord + getBluePyp1 coord + getBluePym1 coord 
      aveBlueNeighbors coord = div (sumBlueNeighbors coord) 5
      getGreenP coord = getGreen (p coord)
      getGreenPxp1 (Coord x y) = getGreen (p (Coord (x+1) y))
      getGreenPxm1 (Coord x y) = getGreen (p (Coord (x-1) y))
      getGreenPyp1 (Coord x y) = getGreen (p (Coord x (y+1)))
      getGreenPym1 (Coord x y) = getGreen (p (Coord x (y-1)))
      sumGreenNeighbors coord = getGreenP coord + getGreenPxp1 coord + getGreenPxm1 coord + getGreenPyp1 coord + getGreenPym1 coord
      aveGreenNeighbors coord = div (sumGreenNeighbors coord) 5
...
instance Transform BlurMany where
  apply (BlurMany 0) pict = pict 
  apply (BlurMany n) pict = apply (BlurMany (n-1)) (apply Blur pict)
------------------------------------------------------------------------------
