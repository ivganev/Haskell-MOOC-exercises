...
statements xs ys = liftA2 (\s t -> s ++ " is " ++ t) xs (liftA2 (++) ["", "not "] ys)
...
calculator func = (<*>) (valfunc func) . readMaybe 

valfunc :: String -> Maybe (Int -> Int)
valfunc "negate" = Just (\x -> (-x))
valfunc "double" = Just (*2)
valfunc _ = Nothing
...
validateDiv x y = liftA2 divide checkedx checkedy 
  where 
    divide x' y' = x' `div` y'
    checkedx = check True "" x
    checkedy = check (y /= 0) "Division by zero!" y
...
validateAddress streetName streetNumber postCode = Address <$> checkedStreetName <*> checkedStreetNumber <*> checkedPostCode
  where 
    checkedStreetName = check (length streetName < 21) "Invalid street name" streetName 
...
boolOrInt s =  checkedBool <|> checkedInt
  where
    checkedInt = check (readInt /= Nothing) "Not an Int" (Right (read s)) 
    checkedBool = check (readBool /= Nothing) "Not a Bool" (Left (read s)) 
    readInt = readMaybe s :: Maybe Int 
    readBool = readMaybe s :: Maybe Bool
...
normalizePhone s = checked1 *> checked2 
  where 
    s1 = filter (/= ' ') s 
    checked1 = check (length s1 < 11) "Too long" s1
    checked2 = traverse checkValidChar s1
    checkValidChar c = check (isDigit c) ("Invalid character: " ++ [c]) c 
...
parseExpression s = if (length (words s)) /= 3 then invalid ("Invalid expression: " ++ s) else checkedExpr
  where 
    a1 = (words s) !! 0 
    op = (words s) !! 1
    a2 = (words s) !! 2
    checkeda1 = (checkDigit a1) <|> (checkVariable a1) -- :: Validation Arg 
    checkeda2 = (checkDigit a2) <|> (checkVariable a2) -- :: Validation Arg 
    checkedExpr = formExpr <$> (checkOp op) <*> checkeda1 <*> checkeda2

formExpr :: String -> Arg -> Arg -> Expression
formExpr op = if op == "+" then Plus else Minus 

checkOp :: String -> Validation String
checkOp s = check (elem s ["+", "-"]) ("Unknown operator: " ++ s) s

checkDigit :: String -> Validation Arg 
checkDigit s = check (all isDigit s) ("Invalid number: " ++ s) (Number (read s))

checkVariable :: String -> Validation Arg 
checkVariable s = check cond ("Invalid variable: " ++ s) (Variable s0)
  where
    cond = ((length s == 1) && (isAlpha s0) && (isLower s0)) 
    s0 = head s
...
instance Functor Priced where
  fmap phi (Priced n x) = Priced n (phi x)

instance Applicative Priced where
  pure = Priced 0
  liftA2 phi (Priced n x) (Priced m y) = Priced (n + m) (phi x y)
...
myFmap phi z = myLiftA2 (\phi x -> phi x) (myPure phi) z
...
tryAll p xs = foldr (<|>) empty (map p xs)
...
instance (Functor f, Functor g) => Functor (Both f g) where
  fmap :: (a -> b) -> Both f g a -> Both f g b
  fmap p (Both z) = Both ((fmap . fmap) p z)
...
instance (Applicative f, Applicative g) => Applicative (Both f g) where
  ...
  liftA2 :: (a -> b -> c) -> Both f g a -> Both f g b -> Both f g c 
  liftA2 p (Both z) (Both w) = Both ((liftA2 . liftA2) p z w)
