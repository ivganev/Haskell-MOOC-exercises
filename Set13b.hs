...
mapM2 op xs ys = mapM f (zip xs ys)
  where f (x,y) = op x y
...
getNeighbors :: Eq a => [(a,[b])] -> a -> [b]
getNeighbors maze place = case lookup place maze of 
  Just ys -> ys 
  Nothing -> []
...
visit maze place = do 
  visited <- get
  let toVisit = [nb | nb <- (getNeighbors maze place), not (elem nb visited)]
  if elem place visited then put visited else put (place:visited) >> (liftM (\x -> ()) $ mapM (visit maze) toVisit)
...
path maze place1 place2 = elem place2 visited
  where visited = snd $ runState (visit maze place1) []
...
findSum2 ks ns= do 
  ...
  -- last line is of type Int -> Int -> [(Int,Int,Int)]

-- findSum2 [] _ = []
-- findSum2 (k:ks) ns = [(k,j,n) | j<-ks, n<-ns, k+j==n]  ++ [(j,k,n) | j<-ks, n<-ns, k+j==n] ++ [(k,k,n)|n<-ns, k+k==n] ++ (findSum2 ks ns)
...
allSums1 :: [Int] -> [Int]
allSums1 xs = map sum $ mapM (\x -> [x,0]) xs

allSums2 :: [Int] -> [Int] 
allSums2 [] = [0]
allSums2 (x:xs) = (map (+x) rest) ++ rest 
  where rest = allSums2 xs

subsequences' :: [Int] -> [[Int]]
subsequences' xs = map concat $ mapM (\x -> [[x], []]) xs 

allSums3 :: [Int] -> [Int]
allSums3 xs = map sum (subsequences' xs)
...
f1 k acc x = if accNew > k then Nothing else Just accNew 
  where accNew = acc + x
...
f2 acc x = do 
  old <- get
  put (x:old)
  return (if elem x old then acc else acc + x)
