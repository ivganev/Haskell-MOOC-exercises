...
add x (Set xs) = if elem x xs then Set xs else Set (sortBy compare (x:xs))
...
data State = Start | Error | Finished | EggsAdded | FlourAddedNoSugar | SugarAddedNoFlour | FlourSugarAdded | Mixed
  deriving (Eq,Show)

step :: State -> Event -> State 
step Start AddEggs = EggsAdded  
step EggsAdded AddFlour = FlourAddedNoSugar 
step EggsAdded AddSugar = SugarAddedNoFlour
step FlourAddedNoSugar AddSugar = FlourSugarAdded 
step SugarAddedNoFlour AddFlour = FlourSugarAdded 
...
reverseNonEmpty (x :| []) = (x :| [])
reverseNonEmpty (x :| xs) = (head revXs) :| ((tail revXs) ++ [x])
...
instance Ord a => Semigroup (Set a) where 
  (Set []) <> (Set ys) = Set ys 
  (Set (x:xs)) <> (Set ys) = add x ((Set xs) <> (Set ys))
...
passwordAllowed p (ContainsSome s) = overlap p s  
passwordAllowed p (DoesNotContain s) = not (overlap p s)
...
data Arithmetic = Ari String Integer
  deriving Show

literal :: Integer -> Arithmetic
literal n = Ari (show n) n

operation :: String -> Arithmetic -> Arithmetic -> Arithmetic
operation op (Ari nStr n) (Ari mStr m) = Ari ("(" ++ nStr ++ op ++ mStr ++ ")") combo
  where 
    combo = case op of 
      "+" -> n+m 
      "*" -> n*m
...
evaluate (Ari nStr n) = n 

render :: Arithmetic -> String
render (Ari nStr n) = nStr
