...
firstHalf s 
    | even (length s)  = take ( (length s) `div` 2 ) s 
    | otherwise        = take ( ((length s) `div` 2) + 1) s
...
joinToLength n strs = [x | a <- strs, b <- strs, let x = a ++ b, length x == n]
...
(+|+) :: [a] -> [a] -> [a]
-- x +|+ y = [x1 | let x1 = safeHead x] ++ [y1 | let y1 = safeHead y]
x +|+ y = [first | (first:_) <- [x,y]]
...
sumRights s = sum $ map (either (\x -> 0) (\x -> x)) s 
...
multiCompose' fs = if null fs then id else head fs . multiCompose' (tail fs)

multiCompose fs 
    | null fs   = id 
    | otherwise = head fs . multiCompose (tail fs)
...
interpreter [] = []
interpreter commands  = case last commands of 
    "printX"    -> interpreter (init commands) ++ [location !! 0]
    "printY"    -> interpreter (init commands) ++ [location !! 1]
    _           -> interpreter (init commands)
  where location = map show (finalPosition (init commands))

finalPosition :: [String] -> [Int]
finalPosition [] = [0,0]
finalPosition commands =  zipWith (+) (finalPosition (init commands)) (increment (last commands))
  where 
    increment cmd = case cmd of 
        "up"    -> [0,1]
        "down"  -> [0,-1]
        "right" -> [1,0]
        "left"  -> [-1,0]
...
interpreter' :: [String] -> [String]
interpreter' = reverse . interpreterReversed . reverse

interpreterReversed :: [String] -> [String]
interpreterReversed []                  = []
interpreterReversed (cmd:commands) = case cmd of 
    "printX"    -> [location !! 0] ++ interpreterReversed commands
    "printY"    -> [location !! 1] ++ interpreterReversed commands
    _           -> interpreterReversed commands  
  where location = map show (finalPosition2 commands)

finalPosition2 :: [String] -> [Int]
finalPosition2 []                    = [0, 0]
finalPosition2 (cmd:commands) = zipWith (+)  (finalPosition2 commands) (increment cmd)
  where 
    increment "up"    = [0,1]
    increment "down"  = [0,-1]
    increment "right" = [1,0]
...
-- ("up":commands)       = zipWith (+) (interpreterHelper commands) [0,1]
-- interpreterHelper ("down":commands)     = zipWith (+) (interpreterHelper commands) [0,-1]
-- interpreterHelper ("right":commands)    = zipWith (+) (interpreterHelper commands) [1, 0]
-- interpreterHelper ("left":commands)     = zipWith (+) (interpreterHelper commands) [-1, 0]
-- interpreterHelper (_:commands)          = interpreterHelper commands
