...
checkNumber (f, l)
  | checkNotNumberString (f++l) = Just (f,l)
  | otherwise = Nothing 

checkNotNumberString :: String -> Bool 
checkNotNumberString s = and $ map (not . isNumber) s
-- checkNotNumberString "" = True 
-- checkNotNumberString s = (not (isNumber (head s)) ) && (checkNotNumberString (tail s))
...
winner scores player1 player2 = do 
  d <- sequence $ map (\p -> lookup p scores) [player1, player2]
  return (if d !! 0 >= d !! 1 then player1 else player2)

winner' :: [(String,Int)] -> String -> String -> Maybe String
winner' scores player1 player2 = 
  (sequence $ map (\p -> lookup p scores) [player1, player2]) ?> check 
  where check d = Just (if d !! 0 >= d !! 1 then player1 else player2)

winner'' :: [(String,Int)] -> String -> String -> Maybe String
winner'' scores player1 player2 = case score1 of 
  Just n -> case score2 of 
    Just m -> if n >= m then Just player1 else Just player2
    Nothing -> Nothing 
  Nothing -> Nothing 
  where 
...
selectSum xs is = do 
  ys <- (sequence $ (map (safeIndex xs) is)) 
  return (sum ys)
...
countAndLog cond xs = fmap sum $ sequence $ map helper xs
  where helper x = if cond x then Logger [show x] 1 else return 0

-- helper :: Show a => (a -> Bool) -> a -> Logger Int
-- helper cond x
--   | cond x    = Logger [show x] 1 
--   | otherwise = Logger [] 0 

countAndLog' :: Show a => (a -> Bool) -> [a] -> Logger Int
countAndLog' cond [] = Logger [] 0 
countAndLog' cond (x:xs)
  | cond x = Logger (r++[show x]) (n+1)
  | otherwise = Logger r n
      where Logger r n = countAndLog' cond xs 
...
balance accountName = BankOp f 
  where 
    f (Bank b) = case Map.lookup accountName b of 
      Just n -> (n,Bank b)
      Nothing -> (0,Bank b)
...
count x = do 
  dict <- get 
  put $ myInsertWith (+) x 1 dict

myInsertWith :: (Eq a, Num b) => (b -> b -> b) -> a -> b -> [(a, b)] -> [(a,b)]
myInsertWith f key dft dict = case lookup key dict of 
  Nothing -> dict++[(key, dft)]
  Just val -> [x | x <- dict, fst x /= key]++[(key,val+1)]

-- checkDict :: Maybe a -> State [(a,Int)] ()
-- checkDict val = case val of 
--   Nothing -> do
--     dict <- get 
--     put (Map.insertWith (+1) )
--   dict <- get 
...
example13 :: State Int Int
example13 = do 
            add 3           -- increment state by 3
