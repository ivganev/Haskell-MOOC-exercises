...
treeMax Empty = 0 
treeMax (Node x r l) = max ( max x (treeMax r) ) (treeMax l)
...
cull val (Node x l r) = if x == val then Empty else Node x lCulled rCulled
  where 
    lCulled = cull val l 
    rCulled = cull val r
...
isOrdered (Node val l r) = isOrdered l && allValues fl l && isOrdered r && allValues fr r 
  where 
    fl = (\x -> x < val) 
    fr = (\x -> x > val)
...
walk (StepL:xs) (Node v l r) = walk xs l 
walk (StepR:xs) (Node v l r) = walk xs r
...
set _ _ Empty = Empty 
set [] x (Node v l r) = Node x l r
set (StepL:xs) y (Node v l r) = Node v (set xs y l) r  
set (StepR:xs) y (Node v l r) = Node v l (set xs y r)  
...
search x (Node v l r) = if x == v then Just [] 
  else
    case (search x l, search x r) of 
