...
rangeOf xs = (maxXs) - (minXs)
    where 
        maxXs = foldr max (head xs) xs
        minXs = foldr min (head xs) xs
...
longest :: Ord a => [[a]] -> [a]
longest xxs = head (sortBy (\xs ys -> compare (head xs) (head ys)) longest_lists)
    where
        longest_lists = [xs | xs <- xxs, length xs == maxLen ]
        maxLen = foldr (\xs m -> max (length xs) m) 0 xxs
------------------------------------------------------------------------------
...
incrementKey key pairs = [(a,bNew) | (a,b) <- pairs, let bNew = if a == key then b+1 else  b  ]
...
winner scores player1 player2 = case comparing f player1 player2 of
    EQ -> player1
    GT -> player1
    LT -> player2
    where
        f p = Map.findWithDefault 0 p scores
...
freqs (x:xs) = Map.alter f x (freqs xs)
    where
        f Nothing = Just 1
        f (Just i) = Just (i+1)
...
transfer from to amount bank 
    ...
    | amount < 0 = bank
    | (bank Map.! from) - amount < 0 = bank
    | otherwise = Map.insertWith (+) to amount $ Map.insertWith (+) from (-amount) bank
...
swap i j arr = arr // [(i,valj), (j, vali)]
    where 
        valj = arr ! j 
        vali = arr ! i
...
maxIndex arr = fst $ head $ sortBy larger $ Data.Array.assocs arr 
    where larger x y = compare (snd y) (snd x)
