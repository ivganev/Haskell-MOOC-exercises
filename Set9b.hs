...
-- prettyPrint n queens = boardList ++ "\n"
--     where 
--         boardList = concat $ tail [ symbol (i,j) | i <- [1..n], j <- [0..n]]
--         symbol (i,0) = "\n"
--         symbol (i,j) = if elem (i,j) queens then "Q" else "."
prettyPrint n [] = concat $ replicate n line
    where line = concat (replicate n ".") ++ "\n"
prettyPrint n (x:xs) =  firstPart ++ "Q" ++ lastPart
    where 
        previous = prettyPrint n xs 
        i = fst x 
        j = snd x
        firstPart = take ((i-1)*(n+1) + (j-1)) previous
        lastPart = drop ((i-1)*(n+1) + j) previous
...
danger _ [] = False 
danger coord (x:xs) = sameRow coord x || sameCol coord x || sameDiag coord x || sameAntidiag coord x || danger coord xs
...
prettyPrint2 n queens = boardList ++ "\n"
    where 
        boardList = concat $ tail [ symbol (i,j) | i <- [1..n], j <- [0..n]]
        symbol (i,0) = "\n"
        symbol c = if elem c queens then "Q" else 
            if danger c queens then "#" else "."
