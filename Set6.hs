...
instance Eq Name where
  (==) :: Name -> Name -> Bool 
  Name x == Name y = if lower x == lower y then True else False 
    where lower = map Data.Char.toLower
...
data RationalNumber = RationalNumber {numer :: Integer, denom :: Integer}
  deriving Show

instance Eq RationalNumber where
  RationalNumber x y == RationalNumber u v = if x*v == u*y then True else False 
...
simplify (RationalNumber x y) = RationalNumber u v 
  where 
    u = x `div` g
    v = y `div` g  
    g = myGcd ((sign x)*x) ((sign y)*y)
    signXY = sign (x*y)
...
instance Num RationalNumber where
  p + q = simplify $ RationalNumber ((numer p)*(denom q) + (numer q)*(denom p)) ((denom p)*(denom q))
  p * q = simplify $ RationalNumber ((numer p)*(numer q)) ((denom p)*(denom q))
  abs q = simplify $ RationalNumber ((sign $ numer q)*(numer q)) ((sign $ denom q)*(denom q))
  signum :: RationalNumber -> RationalNumber 
  signum q = case compare ((numer q)*(denom q)) 0 of
    EQ -> fromInteger 0 
    LT -> fromInteger (-1) 
    GT -> fromInteger 1 
  fromInteger :: Integer -> RationalNumber
  fromInteger x = RationalNumber x 1 
  negate q = simplify $ RationalNumber (-(numer q)) (denom q)
