...
isPalindrome2 str 
    ...
    | otherwise                 = isPalindrome2 (tail (init str)) 
...
safeIndex xs i = if -1 < i && i < length xs then Just ( (!!) xs i ) else Nothing
...
addEithers (Right n) (Left s)    = Left s 
addEithers (Left t) b            = Left t
