...
sumIsLength input output = (length input) === (foldr (\p m ->(snd p) + m) 0 output)
...
inputInOutput input output = forAll (elements input) (\x -> elem x (map fst output) ===  True)
...
frequenciesProp freq (NonEmpty input) = conjoin $ (map (\p -> p input output) [sumIsLength, inputInOutput, outputInInput])
  where 
    output = freq input
...
genList = do 
  l <- choose (3,5)
  fmap sort (sequence (replicate l (elements [0..10])))
...
instance Arbitrary Arg where
  arbitrary = elements ((map Number [0..10]) ++ (map Variable "abczyz"))
