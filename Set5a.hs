...
study s = case s of 
  Freshman -> NthYear 1 
  NthYear n -> if n < 7 then NthYear (n+1) else Graduated 
  _ -> Graduated
...
data UpDown = IncMode Int | DecMode Int 
...
tick :: UpDown -> UpDown
tick (IncMode n) = IncMode (n+1)
tick (DecMode n) = DecMode (n-1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (IncMode n) = DecMode n 
toggle (DecMode n) = IncMode n
...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
rgb (Mix col1 col2) = zipWith (\x y -> (x + y)/2) (rgb col1) (rgb col2) 
...
fromNat :: Nat -> Int
fromNat Zero = 0 
fromNat (PlusOne nat) = (fromNat nat) + 1
...
toBin 0 = O End 
toBin 1 = I End 
toBin n = if parity == 0 then O rest else I rest 
  where 
    parity = n `mod` 2
    rest = toBin (n `div` 2)
