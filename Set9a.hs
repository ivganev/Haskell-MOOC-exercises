...
countValid [] = 0 
countValid (x:xs) = if (cond1 x) || (cond2 x) then 1 + rest else rest
  where 
    rest = countValid xs 
    cond1 x = (x !! 2) == (x !! 4)
    cond2 x = (x !! 3) == (x !! 5)
...
repeated (x:y:zs) = if x == y then Just x else repeated (y:zs) 
repeated _ = Nothing
...
sumSuccess xs = sumSuccessHelper xs (Left "no data")

sumSuccessHelper :: [Either String Int] -> Either String Int -> Either String Int 
sumSuccessHelper [] current = current 
sumSuccessHelper (x:xs) current = case (x, current) of 
  (Right n, Right m) -> sumSuccessHelper xs (Right (n+m))
  (Right n, _) -> sumSuccessHelper xs (Right n)
  (_, _) -> sumSuccessHelper xs current
...
data Lock = LockConstructor {code :: String, isOpen :: Bool}
  deriving Show

-- aLock should be a locked lock with the code "1234"
aLock :: Lock
aLock = LockConstructor {code = "1234", isOpen = False}

-- isOpen returns True if the lock is open
-- isOpen :: Lock -> Bool
-- isOpen (LockConstructor _ b) = b

-- open tries to open the lock with the given code. If the code is
-- wrong, nothing happens.
open :: String -> Lock -> Lock
open attempt l = if attempt == code l then LockConstructor attempt True else l

-- lock closes a lock. If the lock is already closed, nothing happens.
lock :: Lock -> Lock
lock l = if isOpen l then LockConstructor (code l) False else l

-- changeCode changes the code of an open lock. If the lock is closed,
-- nothing happens.
changeCode :: String -> Lock -> Lock
changeCode new l = if isOpen l then LockConstructor new True else l
...
instance Eq Text where 
  Text [] == Text [] = True 
  Text (x:xs) == Text [] = if Data.Char.isSpace x then Text xs == Text [] else False
  Text [] == Text (y:ys) = Text (y:ys) == Text []
  Text (x:xs) == Text (y:ys) = case (Data.Char.isSpace x, Data.Char.isSpace y) of 
    (True, True) -> Text xs == Text ys 
    (True, False) -> Text xs == Text (y:ys) 
    (False, True) -> Text (x:xs) == Text ys 
    (False, False) -> x == y && Text xs == Text ys
...
compose [] _ = []
compose _ [] = []
compose (xy:xys) yzs = case lookup (snd xy) yzs of 
  Just z -> (fst xy, z) : (compose xys yzs)
  Nothing -> compose xys yzs 
...
permute p x = map snd $ sortBy (comparing fst) (zip p x)
